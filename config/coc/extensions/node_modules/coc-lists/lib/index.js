...
var require_america = __commonJS((exports2, module2) => {
  module2["exports"] = function(colors8) {
...
var require_colors = __commonJS((exports2, module2) => {
  ...
  var proto = defineProps(function colors9() {
  }, styles2);
  function applyStyle() {
...
var Colors = class extends import_coc2.BasicList {
  ...
  async loadItems(_context) {
    let {nvim} = this;
    let colors8 = await nvim.eval('split(globpath(&rtp, "colors/*.vim"),"\n")');
    ...
    if (hasPackages) {
      ...
      colors8.push(...packageColors);
    }
    return colors8.map((file) => {
...
")[-1]`);
  ...
  async loadItems(_context) {
    ...
    for (let str of list) {
      ...
      res.push({
        label: str.slice(0, 4) + import_safe2.default.magenta(name) + end,
        filterText: name,
        data: {
          command: name,
    ...
    if (help && import_fs.default.existsSync(help)) {
      ...
      for (let line of content.split(/\r?\n/)) {
        ...
        if (ms) {
          ...
          res.push({
            label: `    ${import_safe2.default.magenta(cmd)}${pad(cmd, 20)}${chars}${pad(chars, 30)}${description}`,
            filterText: cmd,
            data: {
              command: cmd,
              shabang: false,
      ...
      return new Promise((resolve) => {
        ...
        import_fs3.default.readFile(file, "utf8", (err, content) => {
          ...
          for (let line of lines) {
            if (line) {
              let [name, filepath, regex7] = line.split("	");
...
")`);
    ...
    for (let line of list) {
      ...
      if (ms) {
        ...
        res.push({
          label: ` ${import_safe3.default.magenta(mode2)}	${import_safe3.default.blue(key)}	${more}`,
          filterText: `${key} ${more}`,
          data: {
            mode: mode2,
            key
...
var Marks = class {
  ...
  async loadItems(_context) {
    ...
    list = list.slice(1);
    list = list.filter((s) => regex5.test(s));
    return list.map((line) => {
      let [, name, lnum, col, text] = line.match(regex5);
      return {
        label: `${import_safe4.default.magenta(name)}	${text}	${import_safe4.default.grey(lnum)}	${import_safe4.default.grey(col)}`,
  ...
  start(files, cwd) {
    ...
    for (let file of files) {
      ...
      rl.on("line", (line) => {
        ...
        this.emit("data", {
          label: `${import_safe5.default.blue(name)} ${import_safe5.default.grey(relativeFile)}`,
          filterText: name,
          location: {
            uri,
...
var Windows = class {
  ...
  async loadItems(_context) {
    ...
    for (let win of wins) {
      ...
      res.push({
        label: `${import_safe6.default.yellow(win.tabnr.toString())}	${import_safe6.default.yellow(win.winnr.toString())}	${bufname}`,
        data: {id: win.winid}
      });
    }
    return res;
...
var Words = class extends import_coc12.BasicList {
  ...
  async loadItems(context) {
    ...
    if (wordMatch)
      source = `\\b${source}\\b`;
    let regex7 = new RegExp(source, flags);
    let replaceRegex = new RegExp(source, flags + "g");
    for (let line of content.split("\n")) {
      let ms = line.match(regex7);
      if (ms) {
        let range = import_coc12.Range.create(lnum - 1, ms.index, lnum - 1, ms.index + input.length);
        let pre = `${import_safe7.default.magenta(lnum.toString())}${pad(lnum.toString(), total)}`;
        let text = line.replace(replaceRegex, (match) => {
          return import_safe7.default.red(match);
        });
        result.push({
          label: `${pre} ${text}`,
