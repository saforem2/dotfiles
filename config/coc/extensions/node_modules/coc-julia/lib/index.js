...
var Ctx = class {
  ...
  async compileServerSysimg() {
    ...
    await import_coc.workspace.createTerminal({name: "coc-julia-ls"}).then((t) => {
      const cmd = `${bin} --project=${this.compileEnv} ${import_path.default.join(this.compileEnv, "compile.jl")} -s ${this.lsProj} ${this.sysimgDir} ${execfile}`;
      t.sendText(cmd);
    });
  }
  async compileServerBin() {
    ...
    const execfile = await this.resolveExecFile();
    await import_coc.workspace.createTerminal({name: "coc-julia-ls"}).then((t) => {
      const cmd = `${bin} --project=${this.compileEnv} ${import_path.default.join(this.compileEnv, "compile.jl")} -b ${this.lsProj} ${this.serverRoot} ${execfile}`;
      t.sendText(cmd);
    });
  }
  ...
  async startServer() {
    ...
    const serverOptions = {
      command: bin,
      args,
      options: {env: __assign(__assign({}, process.env), {TMPDIR: tmpdir})}
    };
    const clientOptions = {
        ...
        provideCompletionItem: async (document, position, context, token, next) => {
          ...
          if (res && Array.isArray(res.items)) {
            for (const item of res.items) {
              if (item.textEdit && import_coc.TextEdit.is(item.textEdit) && item.kind !== 11) {
